...
float Utils::Magnitude(const sf::Vector2f& vec) // 대각선거리
{
...
{                                           // 매개변수의 대각선거리값으로 매개변수(벡터)를 나눔
    ...
    if (mag != 0)
        vec /= mag;
} // Vector2f 의 x2 + y2 sqrt 가 1로 고정됨
// 벡터값 3,4가 들어갔다면 Magnitude = 5
// 3/5, 4/5 // 0.6, 0.8 
// 0.36 + 0.64 = 1

// ex , x10, y5  Magnitude = sqrt(125)  약 11.18
...
{
    sf::Transform inverse = transformable.getInverseTransform(); // 역변환값 받음
    sf::Vector2f localPoint = inverse.transformPoint(point); // 변환전 포인트를 받아 변환된 포인트로 반환
